...
## How to see what the site looks like
...
4. Visit key routes directly to preview the full experience:
   - `/` (marketing homepage + URL scan input)
   - `/register`, `/login`, `/forgot-password`, `/reset-password`, `/verify-email`
   - `/dashboard` (requires auth)
   - `/pricing`, `/terms`, `/privacy`, `/contact`

### Quick demo path
- Open `/` and submit any URL.
- If not logged in, you will be prompted to register/login.
- After verification, scans redirect to `/scan/[id]` with partial report and upgrade CTA.
